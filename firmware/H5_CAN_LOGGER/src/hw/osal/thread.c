...
/**
 * @brief 코드 설명 
 * - RTOS 의 Thread를 초기화, 생성하는 모듈이며 threadCreate함수를 호출하는 순간 info구조체 내 인덱스가 증가한다.
 * - 실제로 thread를 시작하는 함수는 threadBegin()이다.
 * - Create & Begin 둘다 Mutex를 사용중이며, App단의 System 모듈에서도 다른 Mutex를 만들어 사용하고 있다.
 *   -> 두개의 뮤텍스로 나눈 이유는 간단하게, 인터럽트 등의 예외 상황이 일어났을때를 대비한것
 */
