        ...
        lfs_off_t off, void *buffer, lfs_size_t size)
{
  ...
  ret = spiFlashRead(FS_FLASH_OFFSET + addr, (uint8_t *)buffer, size);
  if (ret != true)
  {
    err = LFS_ERR_IO;
  }
        ...
        lfs_off_t off, const void *buffer, lfs_size_t size)
{
  ...
  ret = spiFlashWrite(FS_FLASH_OFFSET + addr, (uint8_t *)buffer, size);
  if (ret != true)
  {
    err = LFS_ERR_IO;
  }
...
int fsDeviceErase(const struct lfs_config *c, lfs_block_t block)
{
  ...
  ret = spiFlashEraseSector(FS_FLASH_OFFSET + addr);
  if (ret != true) 
  {
    err = LFS_ERR_IO;
  }
