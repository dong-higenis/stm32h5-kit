...
#ifdef _USE_HW_RTOS
#define lock(x)      xSemaphoreTake(mutex_lock[x], portMAX_DELAY);
#define unLock(x)    xSemaphoreGive(mutex_lock[x]);
static SemaphoreHandle_t mutex_lock[I2C_MAX_CH];
#else
#define lock(x)      
#define unLock(x)    
#endif
...
static i2c_tbl_t i2c_tbl[I2C_MAX_CH] = {
  {I2C1, &hi2c1, GPIOB, GPIO_PIN_8,  GPIOB, GPIO_PIN_9,  "I2C1 OLED"},  
};
