...
{
  {GPIOC, GPIO_PIN_0, GPIO_PIN_RESET, NAME_DEF(NORMAL_BTN)},
};


static button_t button_tbl[BUTTON_MAX_CH];
static bool is_enable = true;
static bool is_isr_run = false;
...
void cliButton(cli_args_t *args)
{
  ...
  {
    cliPrintf("isr run : %s\n", is_isr_run ? "True":"False");
    for (int i=0; i<BUTTON_MAX_CH; i++)
    {
      cliPrintf("%d %-12s pin 0x%04X\n",i, buttonGetName(i), button_pin[i].pin);
    }
    ret = true;
  }
  ...
  if (args->argc == 1 && args->isStr(0, "show"))
  {
    while(cliKeepLoop())
    {
      for (int i=0; i<BUTTON_MAX_CH; i++)
      {        
        cliPrintf("%d %-12s %d\n",i, buttonGetName(i), buttonGetPressed(i));
      }
      delay(50);
