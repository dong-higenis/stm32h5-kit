...
bool canMsgRead(uint8_t ch, can_msg_t *p_msg)
{
  ...
  return qbufferRead(&can_tbl[ch].q_msg, (uint8_t *)p_msg, 1);
}
...
uint16_t canGetRxErrCount(uint8_t ch)
{
  ...
  if (HAL_FDCAN_GetErrorCounters(&can_tbl[ch].hfdcan, &error_counters) == HAL_OK)
  {
  ...
  for (int i = 0; i < CAN_MAX_CH; i++)
  {
    ...
    switch (p_can->state)
    {
      case CAN_STATE_IDLE:
        if (p_can->is_open)
        {
          if (p_can->err_code & CAN_ERR_BUS_OFF || canGetTxErrCount(i) >= 8)
          {
            canRecovery(i);
            if (!is_begin[i])
            {
      ...
      case CAN_STATE_WAIT:
        if ((p_can->err_code & CAN_ERR_BUS_OFF) == 0 && canGetTxErrCount(i) < 8)
        {
...
void canRxFifoCallback(uint8_t ch, FDCAN_HandleTypeDef *hfdcan)
{
  ...
  if (HAL_FDCAN_GetRxMessage(hfdcan, can_tbl[ch].fifo_idx, &rx_header, rx_buf->data) == HAL_OK)
  {
    ...
    {
      rx_buf->frame = (rx_header.BitRateSwitch == FDCAN_BRS_ON) ? CAN_FD_BRS : CAN_FD_NO_BRS;
    }
    else
    {
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
  for (int i = 0; i < CAN_MAX_CH; i++)
  {
    if (&can_tbl[i].hfdcan == hfdcan)
    {
      if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
      {
        canRxFifoCallback(i, hfdcan);
      }
...
void cliCan(cli_args_t *args)
{
  ...
  {
    cliPrintf("ch    : 0~%d\n\n", CAN_MAX_CH - 1);
    cliPrintf("mode  : CAN_NORMAL / CAN_MONITOR / CAN_LOOPBACK\n");
    cliPrintf("frame : CAN_CLASSIC / CAN_FD_NO_BRS / CAN_FD_BRS\n");
    cliPrintf("baud  : CAN_100K / CAN_125K / CAN_250K / CAN_500K / CAN_1M / CAN_2M / CAN_4M / CAN_5M\n");
    ret = true;
  }
