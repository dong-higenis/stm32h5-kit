...
#define SPI_CS_H()    gpioPinWrite(SPI_FLASH_CS, _DEF_HIGH)
...
static bool spiFlashReqCmd(uint8_t cmd);
...
bool spiFlashInit(void)
{
  ...
  if (spiFlashGetID(id_tbl, 4) == true)
  {
    if (id_tbl[1] == 0xEF && id_tbl[2] == 0x40 && id_tbl[3] == 0x19)
    {
      ...
      ret = spiFlashReqCmd(ENTER_4BYTE_MODE);
      logPrintf("[%s] Enter 4Byte Mode\r\n", ret ? "OK":"E_");
...
bool spiFlashReqCmd(uint8_t cmd)
{
  bool ret = true;
  uint8_t tx_buf[4];
...
bool spiFlashRead(uint32_t addr, uint8_t *p_data, uint32_t length)
{
  ...
  SPI_CS_L();
  ret &= spiFlashTransfer(tx_buf, NULL, 6, 10);
  if (ret == true)
  {
...
bool spiFlashEraseBlock(uint32_t block_addr)
{
  ...
  /* CMD + 32-bit address (4-byte mode); must send 5 bytes total */
  ret &= spiFlashTransfer(tx_buf, NULL, 5, 10);
  SPI_CS_H();
