...
/**
 * @brief 이번 예제는 SDMMC1과 지금은 별도로 포팅해야하는 Fatfs를 이용하여, SD카드를 읽고 쓰는 예제를 작성해 보겠습니다.
 *
 * @note    main파일이 너무 길어지는 관계로 별도의 파일 "custom_sd.c" "custom_sd.h"를 만들겠습니다.
 *          이번에도 마찬가지로 main파일에 많은 정보가 담겨 가독성이 떨어지는 관계로
 *          cli 함수도 그 파일내에 정의하겠습니다.
 * 
 *          이번에는 초기화 과정을 한눈에 보기위해 기본 code generate 기능은 끄도록 하겠습니다.
 *          ".ioc" 파일에서 상단 메뉴 "Project Manager" -> "Advanced Settings" -> "MX_SDMMC1_SD_Init"
 *          의 "Do Not Generate Function Call"를 체크 해줍시다.
 * 
 * @details 먼저 .ioc에서 SDMMC1을 활성해준 후, Clock Configuration 탭에서 SDMMC1로 들어가는 클럭을
 *          80MHz로 맞춰줍시다.
 *          상세 설정은 .ioc를 참조해 줍시다.
 *          (참고 : SDMMC에 대한 상세한 설정은 "custom_sd.c" 의 Init함수에서 다시 합니다.)
 *          (참고 : gpio input으로 pb4번을 sdmmc1_det핀으로 설정)
 * 
 *          시작하기 이전에, driver 폴더 내 fatfs를 넣어줍니다.
 *          fatfs 를 H5보드에서 사용하기 위해 "diskio.c" 파일의 수정이 필요합니다.(공식 제공 코드는 드라이버 미구현 상태)
 * 
 *          다만 cubeide에서 해당 폴더 인식을 시켜주기 위해 "fatfs" 폴더를 마우스 우클릭 ->
 *          "Add/remove include path"를 통해 빌드시 해당 폴더를 잡아주게끔 해줍시다.
 * 
 *          이제 기본 로직은 "custom_sd.c/h"파일에서 작성을 하도록 하겠습니다.
 *
 */
...
#define _USE_CAN_LOOPBACK_MODE 0
...
static bool    is_fram_inited      = false;
...
int main(void)
{
  ...
  cliAdd("sd",cliSdCard); // cli 등록

  ssd1306Init();

  is_fram_inited = customFramInit(); 

  customFlashInit(); 
  customSdInit(); // sd init
