...
/**
 * @brief 이번 예제는 lwip(lightweight IP)를 포팅하여, Ethernet을 사용하는 예제입니다.
 *        STM32-H5의 경우 공식적으로 MiddleWare내 lwip를 아직 미지원하는듯 하여, 
 *        직접 포팅을 해야 합니다.
 * 
 *        포팅하는 예제는 아래 잘 정리되어있으니 해당 글을 따라한 다음 진행해 주십시오.
 *        https://community.st.com/t5/stm32-mcus/how-to-use-the-lwip-ethernet-middleware-on-the-stm32h5-series/ta-p/691100
 * 
 *        윗 글의 코드 부분은 보지 마시고, 파일만 만든 상태에서 아래 순서대로 읽어주시면 되겠습니다.
 *        1) ethernetif.c (인터럽트 사용 & 핸들러 이름 변경 & 핀 번호 수정)
 *        2) lwip.c 및 lwip.h(직접 작성합니다.)
 *        3) lwip 내 MX_LWIP_Init() 및 MX_LWIP_Process()를 이용해 5초마다 연결 상태를 받아와 출력합니다.
 * 
 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    MX_LWIP_Process(); // lwip 주기적 processing

    // 아래에서 5초에 한번 링크 상태 및 ip를 확인한다.
    static uint32_t pre_time;
    ...
    if (millis() - pre_time > 5000)
    {
      ...
      {
        char ip[16];
        if (lwipGetIPAddress(ip, sizeof(ip)))
        {
          cliPrintf("Link: UP | IP: %s | DHCP: %s\n",
                    ip, lwipGetDHCPStateString());
        }
        else
        {
          cliPrintf("Link: UP | IP: Not assigned | DHCP: %s\n",
                    lwipGetDHCPStateString());
        }
      }
      else
      {
